# 📸 Photos & visuels à fournir — Portfolio Thibault Randrasana

> Checklist de tous les visuels du site. Chaque fichier a un **nom exact** et un
> **dossier de destination** : déposez le fichier au bon endroit avec ce nom
> précis et il s'affiche automatiquement, sans rien d'autre à changer.
>
> Formats conseillés — **Couvertures & galeries** : `.jpg`, paysage, ≥ 1200×800 px.
> **Logos** : `.svg` (ou `.png` transparent). **Portrait** : carré. **PDF** : CV & mémoire.

## ✅ Déjà fournis
- `public/images/profile/thibault-portrait.jpg` — portrait (accueil) ✔
- `public/og.jpg` — carte de partage réseaux sociaux ✔

---

## 1. Logos institutions — bandeau « références » de l'accueil ✅ FOURNIS
*Dossier : `public/images/logos/`*

| # | Organisation | Fichier | Fourni ? |
|---|---|---|---|
| 1 | World Trade Centers Association | `public/images/logos/wtca.jpg` | ✔ |
| 2 | London School of Economics | `public/images/logos/lse.png` | ✔ |
| 3 | Mediterranean Shipping Company (MSC) | `public/images/logos/msc.jpg` | ✔ |
| 4 | Services Industriels de Genève (SIG) | `public/images/logos/sig.jpg` | ✔ |
| 5 | Union européenne | `public/images/logos/eu.jpg` | ✔ |

---

## 2. Projets — 1 couverture + 3 photos de galerie chacun
*Une couverture (visuel de la carte) et jusqu'à 3 photos de détail par projet.*

### Expériences professionnelles

#### Mediterranean Shipping Company (MSC) — Commercial Liner EASTMED — *2025 — en poste*

- **Couverture** → `public/images/experiences/msc-eastmed.jpg`
  ↳ Un porte-conteneurs du service EASTMED (quai, mer, ou terminal du Havre).  ☐
- **Galerie** (jusqu'à 3) :
  1. `public/images/experiences/msc-service-eastmed.jpg` — Carte / visuel du service EASTMED  ☐
  2. `public/images/experiences/msc-service-levant.jpg` — Le service Levant (escales, ligne)  ☐
  3. `public/images/experiences/msc-operations.jpg` — Vous en situation : bureau des opérations, écran de suivi commercial  ☐

#### WTC Antananarivo × WTC Ras Al Khaimah — Project Manager — *2025 — 6 mois*

- **Couverture** → `public/images/experiences/wtc-global-forum.jpg`
  ↳ Une photo du Global Business Forum des World Trade Centers.  ☐
- **Galerie** (jusqu'à 3) :
  1. `public/images/experiences/wtc-1.jpg` — Séance plénière / scène du forum  ☐
  2. `public/images/experiences/wtc-2.jpg` — Networking, stand, ou délégation  ☐
  3. `public/images/experiences/wtc-3.jpg` — Signature / rencontre officielle  ☐

#### Services Industriels de Genève — Stagiaire Gestion de projets / études clients — *2024*

- **Couverture** → `public/images/experiences/sig-geneve.jpg`
  ↳ Locaux / bureaux des SIG à Genève, ou une vue du workboard de suivi des études.  ☐
- **Galerie** (jusqu'à 3) :
  1. `public/images/experiences/sig-geneve-1.jpg` — Le workboard / outil de suivi des études clients  ☐
  2. `public/images/experiences/sig-geneve-2.jpg` — Un « one-pager » de synthèse  ☐
  3. `public/images/experiences/sig-geneve-3.jpg` — Toi aux SIG / la Direction Relations Clients  ☐

### Conseil & études

#### Ministère de l'Industrialisation et du Commerce de Madagascar — Appui à un consultant senior — *2025 — 6 mois*

- **Couverture** → `public/images/experiences/gefp-cover.jpg`
  ↳ Logo du Ministère, ou la couverture du rapport d'étude ZLECAf.  ☐
- **Galerie** (jusqu'à 3) :
  1. `public/images/experiences/gefp-1.jpg` — Atelier / réunion de travail au ministère  ☐
  2. `public/images/experiences/gefp-2.jpg` — La filière textile-habillement malgache (atelier, usine)  ☐
  3. `public/images/experiences/gefp-3.jpg` — Un extrait du rapport ou un graphique clé de l'étude  ☐

#### Galaxam Logistics × USAID — Consultant supply chain & business development — *depuis mai 2026*

- **Couverture** → `public/images/experiences/galaxam-mid-cover.jpg`
  ↳ Optionnel — le diagramme intégré (nuage de points) sert déjà de visuel. Sinon : entrepôt / logistique santé.  ☐
- **Galerie** (jusqu'à 3) :
  1. `public/images/experiences/galaxam-mid-1.jpg` — Entrepôt ou chaîne logistique médicale  ☐
  2. `public/images/experiences/galaxam-mid-2.jpg` — Distribution vers les centres de santé de base  ☐
  3. `public/images/experiences/galaxam-mid-3.jpg` — Vous sur le terrain / avec l'équipe Galaxam  ☐

### Recherche

#### Mémoire de master — Auteur — *2024–2025*

- **Couverture** → `public/images/experiences/master-thesis-cover.jpg`
  ↳ Optionnel — couverture du mémoire. L'essentiel est le PDF (voir §4).  ☐
- **Galerie** (jusqu'à 3) :
  1. `public/images/experiences/master-thesis-1.jpg` — Page de méthodologie ou tableau de données  ☐
  2. `public/images/experiences/master-thesis-2.jpg` — Un graphique / résultat marquant  ☐
  3. `public/images/experiences/master-thesis-3.jpg` — Contexte terrain : port de Madagascar ou Maurice  ☐

### Entrepreneuriat

#### Ploutos — Cofondateur — *2021–2025*

- **Couverture** → `public/images/experiences/ploutos-prototype.jpg`
  ↳ Capture du prototype Ploutos (écran d'accueil).  ☐
- **Galerie** (jusqu'à 3) :
  1. `public/images/experiences/ploutos-1.jpg` — Écran d'une fonctionnalité clé  ☐
  2. `public/images/experiences/ploutos-2.jpg` — Parcours utilisateur / mobile  ☐
  3. `public/images/experiences/ploutos-3.jpg` — Le pitch ou l'équipe fondatrice  ☐

#### PopnBuy OÜ — Fondateur — *2025 — en cours*

- **Couverture** → `public/images/experiences/popnbuy-platform.jpg`
  ↳ Capture de la plateforme PopnBuy (dès qu'elle existe).  ☐
- **Galerie** (jusqu'à 3) :
  1. `public/images/experiences/popnbuy-1.jpg` — Landing page  ☐
  2. `public/images/experiences/popnbuy-2.jpg` — Un parcours produit  ☐
  3. `public/images/experiences/popnbuy-3.jpg` — Vue mobile de l'app  ☐

### Formation

#### INSEEC — Omnes Education — Master Achats et Supply Chain Management — *2025–2026*

- **Couverture** → `public/images/education/inseec-supply-chain.jpg`
  ↳ Logo INSEEC · Omnes Education (ou une photo du campus).  ☐
- **Galerie** (jusqu'à 3) :
  1. `public/images/education/inseec-1.jpg` — Campus / salle INSEEC  ☐
  2. `public/images/education/inseec-2.jpg` — Un projet ou une mise en situation du master  ☐
  3. `public/images/education/inseec-3.jpg` — Diplôme ou attestation  ☐

#### CREA Genève — Omnes Education — Bachelor Business Management — *2024*

- **Couverture** → `public/images/education/crea-geneva.jpg`
  ↳ Logo CREA Genève · Omnes (ou photo du campus).  ☐
- **Galerie** (jusqu'à 3) :
  1. `public/images/education/crea-1.jpg` — Campus CREA Genève  ☐
  2. `public/images/education/crea-2.jpg` — Un projet du bachelor  ☐
  3. `public/images/education/crea-3.jpg` — Diplôme / attestation  ☐

#### London School of Economics — Développement, politiques publiques, données — *2022–2024*

- **Couverture** → `public/images/education/lse.jpg`
  ↳ Logo LSE (réutilisable depuis §1).  ☐
- **Galerie** (jusqu'à 3) :
  1. `public/images/education/lse-1.jpg` — Campus LSE / Londres  ☐
  2. `public/images/education/lse-2.jpg` — Certificat ou intitulé de cours  ☐
  3. `public/images/education/lse-3.jpg` — Un travail / support de cours  ☐

#### HarvardX — Justice Course Certification — *2023*

- **Couverture** → `public/images/education/harvardx.jpg`
  ↳ Logo HarvardX.  ☐
- **Galerie** (jusqu'à 3) :
  1. `public/images/education/harvardx-1.jpg` — Certificat du cours Justice  ☐
  2. `public/images/education/harvardx-2.jpg` — Visuel du cours (Sandel / Harvard)  ☐
  3. `public/images/education/harvardx-3.jpg` — Notes ou support marquant  ☐

### Engagements & éditorial

#### Association MIRAY — Vice-président — *Depuis 2020*

- **Couverture** → `public/images/engagements/miray.jpg`
  ↳ Logo de l'association MIRAY.  ☐
- **Galerie** (jusqu'à 3) :
  1. `public/images/engagements/miray-1.jpg` — Une action associative sur le terrain  ☐
  2. `public/images/engagements/miray-2.jpg` — L'équipe / les bénévoles  ☐
  3. `public/images/engagements/miray-3.jpg` — Un événement organisé  ☐

#### Équipe nationale de basket-ball de Madagascar — Meneur et arrière (U16–U18) — *2015–2016*

- **Couverture** → `public/images/engagements/basket-madagascar.jpg`
  ↳ Photo de l'équipe nationale U16–U18.  ☐
- **Galerie** (jusqu'à 3) :
  1. `public/images/engagements/basket-1.jpg` — Vous en action / en match  ☐
  2. `public/images/engagements/basket-2.jpg` — La sélection nationale (photo d'équipe)  ☐
  3. `public/images/engagements/basket-3.jpg` — Podium, médaille ou moment fort  ☐

#### Journal de l'Étudiant — Rédacteur en chef et chroniqueur — *2023–2024*

- **Couverture** → `public/images/editorial/journal-etudiant.jpg`
  ↳ Logo ou couverture du Journal de l'Étudiant.  ☐
- **Galerie** (jusqu'à 3) :
  1. `public/images/editorial/journal-etudiant-1.jpg` — Une couverture / la une  ☐
  2. `public/images/editorial/journal-etudiant-2.jpg` — Un de vos articles ou chroniques  ☐
  3. `public/images/editorial/journal-etudiant-3.jpg` — La rédaction au travail  ☐

#### Inskahier — INSCAE — Rédacteur en chef — *2022–2023*

- **Couverture** → `public/images/editorial/inskahier.jpg`
  ↳ Logo ou couverture d'Inskahier (INSCAE).  ☐
- **Galerie** (jusqu'à 3) :
  1. `public/images/editorial/inskahier-1.jpg` — Une couverture / un numéro  ☐
  2. `public/images/editorial/inskahier-2.jpg` — Un article publié  ☐
  3. `public/images/editorial/inskahier-3.jpg` — La rédaction / l'équipe  ☐

---

## 3. Documents (pas des images, mais nécessaires)
*Dossier : `public/documents/`*

| Document | Fichier attendu | Fourni ? |
|---|---|---|
| CV — français | `public/documents/cv-fr.pdf` | ☐ |
| CV — anglais | `public/documents/cv-en.pdf` | ☐ |
| Mémoire de master | `public/documents/memoire-master.pdf` | ☐ |

---

## 📊 Récapitulatif
- **Portrait + carte de partage** : 2 (fournis ✔)
- **Logos institutions** : 4
- **Couvertures de projets** : 16
- **Photos de galerie** : 48
- **Documents PDF** : 3
- **Total visuels à fournir** : 68 images + 3 PDF

## 🎯 Si vous ne fournissez que l'essentiel d'abord
1. Les **4 logos** (§1) — les plus visibles.
2. Les couvertures **MSC, WTC, Basket** — celles qui appellent une vraie photo.
3. **CV + mémoire** en PDF (§3).
Le reste (galeries, logos d'écoles) est un bonus ou souvent déjà en votre possession.

## ℹ️ Comment me les transmettre
Les images **collées directement dans le chat** ne me sont pas parvenues jusqu'ici
(seuls le portrait et le .docx sont arrivés comme vrais fichiers). Utilisez la
même méthode d'envoi que pour le portrait ; sinon on trouvera une alternative.

Optionnel — les **galeries** ci-dessus sont facultatives : sans elles, la page
de détail reste complète, elles ajoutent juste de la profondeur.
